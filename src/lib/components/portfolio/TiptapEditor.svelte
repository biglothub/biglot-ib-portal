...
<div class="border border-dark-border rounded-xl overflow-hidden">
	...
	<div class="flex items-center gap-0.5 px-2 py-1.5 border-b border-dark-border bg-dark-surface/50 flex-wrap">
		<button type="button" class={tbClass(isBold)} onclick={() => editor?.chain().focus().toggleBold().run()} title="ตัวหนา">
		...
		<button type="button" class={tbClass(isItalic)} onclick={() => editor?.chain().focus().toggleItalic().run()} title="ตัวเอียง">
		...
		<button type="button" class={tbClass(isStrike)} onclick={() => editor?.chain().focus().toggleStrike().run()} title="ขีดทับ">
		...
		<button type="button" class={tbClass(isOrderedList)} onclick={() => editor?.chain().focus().toggleOrderedList().run()} title="รายการลำดับ">
