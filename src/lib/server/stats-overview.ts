	...
	return [
		{
			...
			rows: [
				...
				{ label: 'Largest Win', value: fmt(wins.length > 0 ? Math.max(...wins) : 0), color: 'text-green-400' },
				{ label: 'Largest Loss', value: fmt(losses.length > 0 ? Math.min(...losses) : 0), color: 'text-red-400' },
